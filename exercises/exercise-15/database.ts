...
export class Database<T> {
    ...
    public async insert(entry: T) {
        this.queue = this.queue.then(() => {
            ...
            return rawData + ExistenceCharacter + JSON.stringify(entry) + '\n';
        }).then((newRawData) => {
            return writeFile(this.filename, newRawData, {encoding: 'utf-8'});
        });
        return this.queue;
    }

    public async delete(query: Filter<T>) {
        this.queue = this.queue.then(() => {
            return readFile(this.filename, {encoding: 'utf-8'})
        }).then((rawData) => {
            let rows = rawData.split('\n');
            for (let rowIndex = 0; rowIndex < rows.length; rowIndex++) {
                const isExist = rows[rowIndex].charAt(0) === ExistenceCharacter;
                if (!isExist) {
                    continue;
                }
                const entry = JSON.parse(rows[rowIndex].substr(1));
                let shouldByDeleted = false;
                if (isMultiFilter(query)) {
                    shouldByDeleted = this.filterByMultiFilter(entry, query);
                } else if (isTextFilter(query)) {
                    shouldByDeleted = this.filterByText(entry, query);
                } else {
                    shouldByDeleted = this.filterByConditionOrArray(entry, query);
                }
                if (shouldByDeleted) {
                    rows[rowIndex] = DeletedCharacter + rows[rowIndex].substr(1);
                }
            }
            return rows.join('\n');
        }).then((newRawData) => {
